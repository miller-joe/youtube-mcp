    ...
    (resolve, reject) => {
      const server = createServer(async (req, res) => {
        try {
          ...
          // Short delay so the browser can render the response before we close the server.
          setTimeout(() => server.close(), 100);
          resolve({
            code,
            redirectUri: `http://127.0.0.1:${(server.address() as import("node:net").AddressInfo).port}/callback`,
          });
        } catch (err) {
          reject(err as Error);
        }
